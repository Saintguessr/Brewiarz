...
function reportSyncError(error) {
  ...
  let message = "Błąd synchronizacji. Sprawdź połączenie z internetem.";
  if (code.includes("permission-denied")) {
    message =
      "Brak uprawnień do zapisu w Firestore (permission-denied). " +
      "Sprawdź: 1) czy w konsoli Firebase włączony jest dostawca logowania " +
      "„Anonymous”, 2) czy reguły Firestore odpowiadają ścieżce sync/{syncId}/days/{day}, " +
      "3) spróbuj wymusić odświeżenie aplikacji (Ctrl/Cmd+Shift+R) — być może działa stara, zbuforowana wersja.";
  }
  window.dispatchEvent(new CustomEvent("brewiarz-sync-error", { detail: { message, code } }));
}
    ...
    const sync = {
      ...
      subscribeDay(dateKey, cb) {
        ...
        return onSnapshot(dayDocRef(sync.syncHash, dateKey), (snap) => {
          ...
          console.error("Nasłuch Firestore nieudany:", e);
          reportSyncError(e);
        });
      },
